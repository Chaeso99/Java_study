		...
		public void routine(String line) 
		{
			...
			if (cmd.equals(ChatProtocol3.ID)) 
			{
				...
				if(mgr.loginChk(cmd, data))//로그인 성공
				{
					...
					{
						sendMessage(ChatProtocol3.ID+ChatProtocol3.MODE+"C"); //이중접속 안내 메시지
					}
					else//로그인 성공, 이중접속 아님
					{
						id = cmd;
						sendMessage(ChatProtocol3.ID+ChatProtocol3.MODE+"T"); //로그인 성공 메시지
						sendAllMessage(ChatProtocol3.CHATLIST+ChatProtocol3.MODE
								+getIdList()); //채팅리스트 띄우기
						sendAllMessage(ChatProtocol3.CHATALL+ChatProtocol3.MODE
								+"["+id+"] 님이 입장하였습니다."); //입장알림 보내기
					}
				}
				else//로그인 실패
				{
					sendMessage(ChatProtocol3.ID+ChatProtocol3.MODE+"F"); //로그인 실패 메시지
				}
			} 
			else if (cmd.equals(ChatProtocol3.CHAT)) // CHAT:bbb;밥먹자
			{
				...
				if (ct != null) 
				{// 상대방과 자신에게 보냄
					ct.sendMessage(ChatProtocol3.CHAT + ChatProtocol3.MODE 
							+ "[" + id + "(S)]" + data); // bbb에게 날라가는것(상대방) , data =
																							// 귓속말
					sendMessage(ChatProtocol3.CHAT + ChatProtocol3.MODE 
							+ "[" + id + "(S)]" + data); // 자신(aaa)에게 날라옴(sendMessage)
				} 
				else 
				{// 자신에게 보내는것 (aaa)
			...
			{
				Vector<MessageBean3> vlist = mgr.getMsgList(id); //나와 관련된 쪽지들 다 가져와서 벡터로 저장
				String str = "";
